...
def carregar_chave_openai():
    ...
    if not api_key: 
        ...
        except Exception as e: st.warning(f"Problema ao acessar st.secrets para OpenAI: {e}")
    if not api_key:
        st.error("Chave API da OpenAI não configurada. Defina OPENAI_API_KEY.")
        return None
    return api_key
...
def reescrever_texto_com_openai(texto_anonimizado: str, prompt_instrucao_usuario: str, system_prompt: str, api_key: str) -> str | None:
    ...
    try:
        ...
        st.error(f"Ocorreu um TypeError ao tentar usar a API da OpenAI: {te}. Isso pode estar relacionado à forma como os proxies são configurados ou uma incompatibilidade de versão da biblioteca.")
    except Exception as e: st.error(f"Ocorreu um erro inesperado ao processar com a LLM (OpenAI): {e}")
    return None
