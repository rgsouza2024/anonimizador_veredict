...
# Versão 1.0 (Beta) - Integração LLM Google Gemini 2.0 Flash Lite.
...
import tiktoken # Mantido para contagem de tokens, embora a tokenização do Gemini seja diferente.
                  # Para uma contagem precisa para Gemini, a API do Gemini teria seus próprios métodos.
                  # Por ora, tiktoken pode dar uma estimativa genérica.
...
# Prompts para Gemini (adaptados do seu prompt anterior)
...
# --- Chaves para os widgets e session state (sufixo _v3 para esta versão) ---
KEY_TEXTO_ORIGINAL_AREA = "texto_original_input_area_v3"
KEY_TEXTO_ANONIMIZADO_OUTPUT_AREA_STATE = "texto_anonimizado_output_area_state_v3"
KEY_OUTPUT_PDF_ANONIMIZADO_DISPLAY_STATE = "output_pdf_anonimizado_display_state_v3" 
KEY_PDF_UPLOADER = "pdf_uploader_v3"
KEY_BOTAO_ANONIMIZAR_PDF = "botao_anonimizar_pdf_v3"
KEY_BOTAO_APAGAR_AREA = "botao_apagar_area_v3"
KEY_BOTAO_ANONIMIZAR_AREA = "botao_anonimizar_area_v3"
KEY_COPY_BTN_FILE = "copy_btn_file_v3"
KEY_COPY_BTN_AREA = "copy_btn_area_v3"
KEY_TEXTO_ORIGINAL_PDF_DISPLAY = "texto_original_pdf_exp_v3"
KEY_LLM_REWRITE_BTN_PDF = "llm_rewrite_pdf_v3"
KEY_LLM_REWRITE_BTN_AREA = "llm_rewrite_area_v3"
KEY_LLM_OUTPUT_PDF_STATE = "llm_output_pdf_state_v3"
KEY_LLM_OUTPUT_AREA_STATE = "llm_output_area_state_v3"
KEY_COPY_LLM_PDF = "copy_llm_pdf_v3"
KEY_COPY_LLM_AREA = "copy_llm_area_v3"
KEY_TEXTO_EXTRAIDO_PDF_CONTAGEM = "texto_extraido_pdf_contagem_v3"
KEY_NUM_TOKENS_PDF_EXTRAIDO = "num_tokens_pdf_extraido_v3"
...
def carregar_lista_de_arquivo(nome_arquivo):
    ...
    try:
        ...
        st.warning(f"Arquivo de lista '{nome_arquivo}' não encontrado.")
    except Exception as e:
        st.error(f"Erro ao ler o arquivo '{nome_arquivo}': {e}")
    return lista_itens

# --- Listas Estáticas (Conforme seu último código) ---
...
# --- Configuração e Inicialização do Presidio (Mesma lógica da versão anterior) ---
@st.cache_resource
...
# --- Função para Contar Tokens (usando tiktoken, pode dar uma estimativa) ---
def contar_tokens_para_estimativa(texto: str) -> int:
    if not texto:
        return 0
    try:
        encoding = tiktoken.get_encoding("cl100k_base") # Encoder comum para modelos GPT
    except Exception:
        return len(texto.split()) # Fallback muito grosseiro
    num_tokens = len(encoding.encode(texto))
    return num_tokens

# --- Função para carregar chave da API do Google (Gemini) ---
...
# --- Função para chamar a LLM Google Gemini ---
def reescrever_texto_com_gemini(texto_anonimizado: str, prompt_instrucao_usuario: str, system_prompt: str, api_key: str) -> str | None:
    ...
    try:
        ...
        generation_config = genai.types.GenerationConfig(
            # candidate_count=1, # Padrão é 1
            # stop_sequences=['...'], # Se necessário
            max_output_tokens=8192, # Gemini 2.0 Flash Lite tem um limite alto, mas a API pode ter um default menor para output. 8192 é um valor comum.
            temperature=0.3,
            # top_p=0.95, # Descomente se quiser usar
            # top_k=40    # Descomente se quiser usar
        )
        
        safety_settings = [ # Configurações de segurança para evitar bloqueios comuns
        ...
        model = genai.GenerativeModel(
            model_name="gemini-2.0-flash-lite", # Usando o modelo Flash mais recente
            system_instruction=system_prompt,
            generation_config=generation_config,
            safety_settings=safety_settings
        )
        
        # O prompt do Gemini é mais simples, apenas o conteúdo do usuário.
        # A instrução do sistema e as instruções de tarefa já foram passadas.
        ...
        if response.parts:
            # Para respostas de texto simples, geralmente está em response.text
            if hasattr(response, 'text') and response.text:
                return response.text.strip()
            else: # Fallback para iterar sobre as partes se .text não estiver preenchido
                texto_reescrito_partes = [part.text for part in response.parts if hasattr(part, 'text')]
                return "".join(texto_reescrito_partes).strip() if texto_reescrito_partes else None
        elif hasattr(response, 'text') and response.text: # Checagem adicional se não houver parts
             return response.text.strip()

        st.error("A LLM (Gemini) não retornou uma resposta de conteúdo válida.")
        return None

    except ValueError as ve: # Captura erros de prompt bloqueado que podem vir como ValueError
        if "prompt was blocked" in str(ve).lower() or (hasattr(ve, 'args') and ve.args and "prompt was blocked" in str(ve.args[0]).lower()):
        ...
        st.error(f"Ocorreu um erro inesperado ao processar com a LLM (Gemini): {e}")
    return None
...
with tab_pdf:
    ...
    if arquivo_pdf_carregado is not None:
        ...
        if st.session_state.get(KEY_NUM_TOKENS_PDF_EXTRAIDO, 0) > 0:
            st.info(f"Texto extraído do PDF ('{st.session_state.nome_arquivo_carregado}'): Aproximadamente **{st.session_state[KEY_NUM_TOKENS_PDF_EXTRAIDO]} tokens** (Estimativa Genérica).")
    ...
    if texto_anonimizado_pdf_atual:
        ...
        if texto_llm_pdf_atual:
            ...
            st.caption("Clique no botão acima para gerar um resumo jurídico do texto anonimizado.")
...
with tab_texto:
    ...
    if texto_anonimizado_area_atual and texto_anonimizado_area_atual not in ["O resultado da anonimização aparecerá aqui...", "O resultado da área de texto aparecerá aqui...", "Erro ao processar o texto da área."]:
        ...
        if texto_llm_area_atual:
            st.text_area("Texto Reescrito com Inteligência Artificial:", value=texto_llm_area_atual, height=300, disabled=True)
...
st.sidebar.markdown(
    ...
    "2. **Gere um Resumo Jurídico com IA (Opcional):**\n"
    "   - Após a anonimização, se desejar um resumo jurídico detalhado do texto (sem as tags de anonimização explícitas),"
